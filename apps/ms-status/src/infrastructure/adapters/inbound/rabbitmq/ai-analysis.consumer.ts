  ...
  private async connectWithRetry(retries = 5, delayMs = 5000) {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        this.logger.log(`Conexión exitosa con RabbitMQ y cola activada.: ${this.queueName}`);

        await this.channel.consume(this.queueName, async (msg) => {
          if (msg) {
            try {
              const content = JSON.parse(msg.content.toString());
              this.logger.log(`Análisis de IA recibido para queja: ${content.complaintId}, Urgency: ${content.urgency}`);

              await this.statusService.updateUrgency(content.complaintId, content.urgency);

              this.channel?.ack(msg);
            } catch (err) {
              this.logger.error(`Error al procesar el mensaje de RabbitMQ: ${(err as Error).message}`);
        ...
        this.logger.warn(`No se pudo conectar con RabbitMQ. (attempt ${i + 1}/${retries}): ${(error as Error).message}`);
        if (i < retries - 1) {
    ...
    this.logger.error('No se pudo conectar a RabbitMQ tras varios intentos. El servicio continuará sin el consumidor de RabbitMQ.');
  }
