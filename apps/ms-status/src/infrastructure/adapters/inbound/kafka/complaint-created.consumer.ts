      ...
      await this.consumer.run({
        eachMessage: async ({ topic, partition, message }) => {
          ...
          try {
            ...
            this.logger.log(`Received message on ${topic}: complaintId=${payload.complaintId}`);

            // Initialize the case in the status database
            await this.statusService.initializeCase(payload.complaintId);
            
          } catch (error) {
            this.logger.error(`Error processing Kafka message: ${(error as Error).message}`);
            // In a production system, we would publish to a Dead Letter Queue (DLQ) here
          }
        },
      });
    } catch (error) {
      this.logger.error(`Failed to connect to Kafka broker: ${(error as Error).message}`);
    }
  }
